...
function toneClasses(tone: StatusTone) {
  if (tone === "success") {
    return {
      badge:
        "border-success/15 bg-success-light text-success-hover [&_.status-dot]:bg-success",
  ...
  if (tone === "warning") {
    return {
      badge:
        "border-warning/20 bg-warning-light text-warning-hover [&_.status-dot]:bg-warning-hover",
  ...
  return {
    badge:
      "border-danger/15 bg-danger-light text-danger [&_.status-dot]:bg-danger",
  ...
  return (
    ...
    >
      ...
      <rect
        ...
        strokeWidth="2"
      />
      <rect x="174" y="48" width="12" height="12" rx="3" fill="currentColor" fillOpacity="0.12" />
      <rect x="194" y="48" width="12" height="12" rx="3" fill="currentColor" fillOpacity="0.12" />
      <rect x="174" y="68" width="12" height="12" rx="3" fill="currentColor" fillOpacity="0.12" />
      <path
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#f9fafb] p-6">
        <div className="w-full max-w-md rounded-2xl border border-slate-200/80 bg-white p-8 text-center shadow-md shadow-slate-900/5">
          ...
          <Button className="mt-6 h-10 w-full rounded-xl text-sm font-semibold" onClick={() => loadOrder()}>
            Coba Lagi
          </Button>
        </div>
      </main>
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#f9fafb]">
      ...
      <div className="relative border-b border-slate-200/80 bg-white/90 backdrop-blur">
        <div className="mx-auto flex h-16 w-full max-w-4xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <Button asChild variant="ghost" className="-ml-3 h-10 rounded-xl text-sm font-semibold text-slate-600 hover:text-primary">
            <Link href="/">
        ...
        <section className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-md shadow-slate-900/5 sm:p-6 lg:p-8">
          ...
          <div className="relative grid gap-6 xl:grid-cols-[1fr_300px] xl:items-start">
            <div>
              ...
              <p className={`mt-5 text-[11px] font-medium uppercase tracking-wider ${statusTone.label}`}>
                {statusCopy.eyebrow}
              </p>
            ...
            <aside className={`relative rounded-xl border p-4 sm:max-w-sm xl:max-w-none ${statusTone.softPanel}`}>
              <div className={`mb-5 flex h-12 w-12 items-center justify-center rounded-xl ring-4 ${statusTone.icon}`}>
              ...
              <dl className="space-y-4 text-sm">
                ...
                <div className="grid grid-cols-2 gap-3 border-t border-slate-200/80 pt-4">
                  ...
                  <div>
                    <dt className="text-xs text-slate-500">Batas bayar</dt>
                    <dd className="mt-1 text-sm font-medium text-slate-900">
              ...
              <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-xs text-slate-500">
                    Event ID: <span className="font-mono">{orderData.order.event_id}</span>
                  </p>
                </div>
                <StatusPill status={orderData.order.status} />
              </div>
              ...
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm shadow-slate-900/5 sm:p-5">
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      Provider, metode, dan status pembayaran order.
                    </p>
                  </div>
                </div>
                <dl className="grid gap-3 text-sm sm:grid-cols-2">
                  <div className="rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-2">
                    ...
                    <dd className="mt-1 font-medium text-slate-900">{orderData.payment?.status || "-"}</dd>
                  </div>
                  <div className="rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-2">
                    <dt className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Provider</dt>
                    <dd className="mt-1 font-medium text-slate-900">{orderData.payment?.provider || "-"}</dd>
                  </div>
                  <div className="rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-2">
                    <dt className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Metode</dt>
                    <dd className="mt-1 font-medium text-slate-900">{orderData.payment?.method || "-"}</dd>
                  </div>
                  <div className="rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-2">
                    <dt className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Dibayar</dt>
                    <dd className="mt-1 font-medium text-slate-900">
                      {formatDateTime(orderData.payment?.paid_at || orderData.order.paid_at)}
                    </dd>
                  </div>
                </dl>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm shadow-slate-900/5 sm:p-5">
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      {orderData.participants?.length ?? 0} peserta tercatat pada order ini.
                    </p>
                  </div>
                </div>
                {orderData.participants && orderData.participants.length > 0 ? (
                  <div className="divide-y divide-slate-100">
                    {orderData.participants.map((participant, index) => (
                      <div key={`${participant.ticket_category_id}-${index}`} className="py-3 first:pt-0 last:pb-0">
                  ...
                  <div className="rounded-xl border border-dashed border-slate-200/80 bg-slate-50/80 p-4 text-sm leading-relaxed text-slate-600">
                    Data peserta belum tersedia untuk order ini.
                  </div>
                )}
              </div>
            </section>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-relaxed text-slate-600">
                      Lanjutkan ke checkout Xendit untuk menyelesaikan pembayaran order ini.
                    </p>
                  </div>
                  <Button
