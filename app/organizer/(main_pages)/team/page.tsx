...
const getMemberLabel = (member: OrganizerTeamMember) =>
  member.name || member.email || member.user_email || (member.user_id ? `User ${member.user_id}` : "Support member");
...
export default function OrganizerTeamPage() {
  ...
  const activeSupportCount = useMemo(
    () => supportMembers.filter((member) => member.status === "active").length,
    [supportMembers],
  );
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(error, "Gagal mengambil daftar support staff."));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleAddMember = async () => {
    ...
    if (inviteMode === "user_id" && !trimmedUserId) {
      toast.error("Isi user ID support member.");
      return;
    }

    const parsedUserId = Number(trimmedUserId);
    if (inviteMode === "user_id" && (!Number.isInteger(parsedUserId) || parsedUserId <= 0)) {
      toast.error("User ID harus berupa angka positif.");
      return;
    }
    ...
    try {
      ...
      setEmail("");
      setUserId("");
      toast.success("Support member added. Invitation email has been sent.");
      await loadMembers();
    } catch (error) {
      toast.error(getApiErrorMessage(error, "Gagal menambahkan support member."));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleRevoke = async (member: OrganizerTeamMember) => {
    if (!window.confirm(`Revoke support access for ${getMemberLabel(member)}?`)) {
      return;
    }
    ...
    try {
      ...
      toast.error(getApiErrorMessage(error, "Gagal mencabut akses support member."));
    } finally {
      setRevokingId(null);
    }
  };
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-600">
              Undang assistant untuk membantu scan tiket dan mengelola komunitas.
            </p>
          </div>
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm leading-relaxed text-slate-500">
                Backend will send the invitation email after this member is added.
              </p>
            </div>
          </div>
          ...
          <div className="mt-5 rounded-xl border border-slate-200/80 bg-slate-50/80 p-3">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              ...
              <button
                type="button"
                onClick={() => handleInviteModeChange("user_id")}
                className={cn(
                  "flex h-10 items-center justify-center gap-2 rounded-xl border px-3 text-sm font-semibold transition",
                  inviteMode === "user_id"
                    ? "border-primary/20 bg-white text-primary shadow-sm shadow-slate-900/5"
          ...
          <div className="mt-5 space-y-4">
            {inviteMode === "email" ? (
              <div className="space-y-2">
                <Label htmlFor="support-email">Support member email</Label>
                <Input
                  id="support-email"
                  type="email"
                  value={email}
              ...
              <div className="space-y-2">
                <Label htmlFor="support-user-id">Support member user ID</Label>
                <Input
                  id="support-user-id"
                  inputMode="numeric"
                  value={userId}
          ...
          <div className="flex items-center justify-between gap-4 border-b border-slate-200/80 px-5 py-4">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Active and revoked support members are shown here.
              </p>
            </div>
          ...
          {loading ? (
                  ...
                  {supportMembers.map((member) => (
                    <tr key={member.id} className="hover:bg-slate-50/70">
                      <td className="px-5 py-4">
                        <p className="font-semibold text-slate-950">{getMemberLabel(member)}</p>
                        <p className="text-xs text-slate-500">
                          {member.email || member.user_email || (member.user_id ? `User ID ${member.user_id}` : "-")}
                        </p>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4 text-slate-500">{formatDate(member.created_at)}</td>
                      <td className="px-5 py-4 text-right">
                        <Button
                          ...
                          disabled={member.status === "revoked" || revokingId === member.id}
            ...
            <div className="px-6 py-14 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">
                Add a support member to help with event operations.
              </p>
            </div>
          )}
        </section>
