...
export function SkeletonOrganizerEventCard({ layout = "list" }: { layout?: "list" | "grid" }) {
  const isGrid = layout === "grid";

  return (
    <div className={`flex bg-card relative text-card-foreground border border-border rounded-2xl overflow-hidden shadow-sm h-full ${isGrid ? "flex-col" : "flex-col sm:flex-row"}`}>
      {/* Image Skeleton */}
      <div className={`relative shrink-0 flex justify-center items-center z-10 overflow-hidden ${isGrid ? "m-3 rounded-[12px] w-[calc(100%-24px)] h-48" : "m-3 sm:m-4 sm:mr-0 rounded-[12px] w-[calc(100%-24px)] sm:w-70 h-48 sm:min-h-55"}`}>
...
export function SkeletonOrganizerEvents({ layout = "list" }: { layout?: "list" | "grid" }) {
  return (
