...
const getErrorStatus = (error: unknown) => (error as AxiosError).response?.status;
...
function PostDialog({
  ...
  onSubmit: (payload: { title: string; body: string; post_type: "text" | "announcement" }) => Promise<void>;
}) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-2xl sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{post ? "Edit Post" : "Buat Post"}</DialogTitle>
        </DialogHeader>
          ...
          {!post ? (
            <div className="space-y-2">
              ...
              <select
                ...
                value={postType}
                onChange={(event) =>
                  setPostType(event.target.value as "text" | "announcement")
                }
...
function CommentPanel({ communityId, post }: { communityId: string; post: Post }) {
  ...
  const loadComments = useCallback(async () => {
    setLoading(true);
    try {
      const result = await CommunityService.listComments(communityId, post.id, 1, 50);
  ...
  const handleCreate = async () => {
    if (!body.trim()) return;
    try {
      const created = await CommunityService.createComment(communityId, post.id, {
        body: body.trim(),
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      await CommunityService.deleteComment(communityId, post.id, commentToDelete.id);
      setComments((current) => current.filter((item) => item.id !== commentToDelete.id));
  ...
  return (
    <div className="mt-4 rounded-xl border border-slate-200/80 bg-slate-50/70 p-3">
      ...
      <div className="mt-3 flex gap-2">
        ...
        />
        <Button className="h-10 rounded-xl" onClick={handleCreate} disabled={!body.trim()}>
      ...
      <div className="mt-3 space-y-2">
        {comments.map((comment) => (
          <div key={comment.id} className="rounded-xl border border-slate-200/80 bg-white p-3">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-xs font-semibold text-slate-500">
                  {comment.author_name || `User ${comment.author_user_id}`} - {formatDate(comment.created_at)}
                </p>
        ...
        {!loading && comments.length === 0 ? (
          <p className="py-3 text-center text-xs text-slate-500">Belum ada komentar.</p>
        ) : null}
      </div>
        ...
        <DialogContent className="rounded-2xl sm:max-w-md">
          ...
          <div className="space-y-3">
            <p className="text-sm leading-6 text-slate-600">
              Komentar dari {commentToDelete?.author_name || "user ini"} akan dihapus dari post ini.
            </p>
            {commentToDelete ? (
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">
                <p className="line-clamp-3 whitespace-pre-line">{commentToDelete.body}</p>
              </div>
            ) : null}
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              disabled={Boolean(deletingCommentId)}
            >
              {deletingCommentId ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
              Hapus
            </Button>
          </DialogFooter>
        </DialogContent>
...
export default function SupportCommunityClient() {
  ...
  const loadCommunity = useCallback(async () => {
    ...
    try {
      ...
      if (result.posts.length) {
        ...
        const postResult = await CommunityService.listPosts(result.community.id, 1, 50);
      ...
      else toast.error(getApiErrorMessage(error, "Gagal mengambil komunitas support."));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handlePostSubmit = async (payload: {
    ...
    try {
      if (editingPost) {
        const updated = await CommunityService.updatePost(community.id, editingPost.id, {
          title: payload.title,
          body: payload.body,
        });
        setPosts((current) =>
          current.map((post) => (post.id === editingPost.id ? updated : post)),
        );
        toast.success("Post diperbarui.");
        return;
      }

      const created = await CommunityService.createPost(community.id, payload);
  ...
  const handleDeletePost = async (post: Post) => {
    if (!community || !window.confirm(`Hapus post "${post.title}"?`)) return;
    try {
      await CommunityService.deletePost(community.id, post.id);
      setPosts((current) => current.filter((item) => item.id !== post.id));
      setCommunity((current) =>
        current ? { ...current, post_count: Math.max(current.post_count - 1, 0) } : current,
      );
        ...
        <>
          <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-900/5">
            {community.banner_url ? (
              <div className="relative h-44 bg-slate-100 md:h-56">
                <Image src={community.banner_url} alt="" fill className="object-cover" />
              ...
              <div className="p-5">
                ...
                <h1 className="mt-2 text-2xl font-bold text-slate-950">{community.name}</h1>
              </div>
            )}
          ...
          <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm shadow-slate-900/5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-base font-semibold text-slate-950">Community posts</h2>
          ...
          {sortedPosts.length ? (
              ...
              {sortedPosts.map((post) => (
                ...
                >
                  ...
                  >
                    ...
                    {expandedPostId === post.id ? "Tutup komentar" : "Kelola komentar"}
                  </Button>
                  {expandedPostId === post.id ? (
                    <CommentPanel communityId={community.id} post={post} />
                  ) : null}
                </article>
              ))}
            </section>
            ...
            <section className="rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-14 text-center shadow-sm shadow-slate-900/5">
              ...
              <h2 className="mt-4 text-lg font-semibold text-slate-950">Belum ada post</h2>
