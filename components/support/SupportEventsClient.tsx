...
const getEventId = (event: OrganizerEventCardType) =>
...
export default function SupportEventsClient() {
    ...
    const [checkedInCounts, setCheckedInCounts] = useState<
    ...
    const visibleStart = total === 0 ? 0 : (page - 1) * PAGE_LIMIT + 1;
    const visibleEnd = Math.min(page * PAGE_LIMIT, total);
    ...
    const totalCheckedIn = useMemo(
        () =>
            Object.values(checkedInCounts).reduce(
                (sum, value) => sum + value,
                0,
            ),
        [checkedInCounts],
    ...
    const activeEvents = useMemo(
        () =>
            events.filter(
                (event) => !closedStatuses.has(toStatusKey(event.status)),
            ).length,
        [events],
    );

    const fetchCheckInCounts = useCallback(
        async (items: OrganizerEventCardType[]) => {
            ...
            const entries = await Promise.all(
                items.map(async (event) => {
                    ...
                    try {
                        const response =
                            await EventService.getOrganizerCheckInHistory(
                                eventId,
                                {
                                    page: 1,
                                    limit: 1,
    ...
    const loadEvents = useCallback(async () => {
        ...
        try {
            const result = await SupportService.getSupportEvents({
                q: query,
                status,
            ...
            await fetchCheckInCounts(result.data);
        } catch (error) {
    ...
    if (accessDenied) {
        return (
            <AccessMessage
                ...
                message="Akses support belum tersedia untuk akun ini."
            />
        );
    }
                ...
                <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-600">
                            Pilih event yang ditugaskan, buka scanner, dan
                            pantau progress kehadiran peserta.
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="mt-0.5 text-sm text-slate-500">
                                {loading
                                    ? "Memuat event..."
                                    : `${visibleStart}-${visibleEnd} dari ${total.toLocaleString("id-ID")} event`}
                            </p>
                        </div>
                    </div>
                        ...
                        {events.map((event) => {
                            ...
                            return (
                                <OrganizerEventCard
                                    ...
                                    mode="check-in"
                                    checkedInCount={
                                        checkedInCounts[eventId] ?? 0
                                    }
                                    onCheckIn={() =>
                                        router.push(
                                            `/dashboard/support/check-in/${eventId}?support=1`,
                                        )
                                    }
                                />
                            );
                ...
                <section className="rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-14 text-center shadow-sm shadow-slate-900/5">
                    ...
                    <p className="mt-2 text-sm text-slate-500">
                        {query
                            ...
                            : "Event support yang ditugaskan akan muncul di sini dan siap dipakai untuk validasi tiket."}
                    </p>
                </section>
            )}
        </SupportPageSurface>
