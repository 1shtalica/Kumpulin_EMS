    ...
    switch (normalizeStatus(status)) {
        case "paid":
            return {
                ...
                badgeClassName:
                    "border-success/15 bg-success-light text-success-hover",
            };
        case "awaiting_payment":
            return {
                ...
                badgeClassName:
                    "border-warning/20 bg-warning-light text-warning-hover",
            };
        case "expired":
            return {
                label: "Kedaluwarsa",
        ...
        return (
            <div className="flex flex-wrap gap-2">
                <Button
                    disabled
                    variant="outline"
                    className="h-10 rounded-xl border-slate-200 bg-white text-sm font-semibold text-slate-400"
                >
                    Pesanan Kedaluwarsa
                </Button>
                <Button
                ...
                <div className="min-w-0">
                    ...
                    <div className="mt-4 grid gap-3 text-sm text-slate-600 md:grid-cols-3">
                        ...
                        <div className="rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-2">
                            ...
                            <p className="mt-1 text-slate-700">
                                {formatDateTime(order.paid_at ?? null)}
                            </p>
                        </div>
                    </div>
                </div>
...
function MyOrdersPageContent() {
    ...
    const selectedStatus: StatusFilter =
        statusParam && VALID_FILTERS.has(statusParam) ? statusParam : "all";
    ...
    const selectedTab = useMemo(
        () =>
            STATUS_TABS.find((tab) => tab.value === selectedStatus) ??
            STATUS_TABS[0],
        [selectedStatus],
    );
    ...
    const summaryText = useMemo(() => {
        if (pagination.total_items <= 0)
            return "Tidak ada pesanan ditampilkan.";
                    ...
                    <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                        <div>
                            ...
                            <p className="mt-2 max-w-2xl text-xs leading-relaxed text-slate-600 md:text-sm">
                                Pantau pembelian tiket, status pembayaran, dan
                                lanjutkan checkout Xendit yang belum selesai.
                            </p>
                        </div>
                        <Button
                            type="button"
                    ...
                    {!isLoading && !errorMessage && items.length === 0 ? (
                        <div className="rounded-2xl border border-slate-200/80 bg-white p-10 text-center shadow-sm shadow-slate-900/5">
                            ...
                            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
                                Pesanan tiket Anda akan muncul di sini setelah
                                checkout dimulai.
                            </p>
