...
export const EventService = {
    ...
    async getEventById(id: string): Promise<Event> {
        try {
            const response = await axiosClient.get<EventResponse>(
                `/events/${id}`,
            );
    ...
    async getEventByIdFull(id: string): Promise<Event | null> {
        try {
            ...
            console.error(
                `Failed to fetch full event details for ${id}:`,
                error,
            );
            return null;
        }
    ...
    async updateEventLocation(
        ...
        try {
            await axiosClient.patch(
                `/organizer/events/${eventId}/address`,
                payload,
            );
        } catch (error: any) {
